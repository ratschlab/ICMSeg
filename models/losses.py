    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        dice_loss = (
            ...
            - (
                ...
                / (mod_a + mod_b + self.eps + self.smooth)
            ).mean()
        )
...
class NTXentLossSegm(MemoryBankModule):
    ...
    def __init__(
        ...
        super(NTXentLossSegm, self).__init__(size=memory_bank_size)
    ...
    def forward(self, out0: torch.Tensor, out1: torch.Tensor):
        ...
        out1, negatives = super(NTXentLossSegm, self).forward(
        ...
        if negatives is not None:
            ...
            logits_00 = torch.einsum("ncij,mcij->nmij", out0, out0_large) / self.temperature
            logits_01 = torch.einsum("ncij,mcij->nmij", out0, out1_large) / self.temperature
            logits_10 = torch.einsum("ncij,mcij->nmij", out1, out0_large) / self.temperature
            logits_11 = torch.einsum("ncij,mcij->nmij", out1, out1_large) / self.temperature
            
            # remove simliarities between same views of the same image
            logits_00 = logits_00[~diag_mask].view(batch_size, -1, h, w)
            logits_11 = logits_11[~diag_mask].view(batch_size, -1, h, w)
            ...
            if self.gather_distributed:
                labels = labels + dist.rank() * batch_size * h * w
            labels = labels.repeat(2*h*w).reshape(h, w, batch_size*2).transpose(1, 2).transpose(0, 1)
            # 
...
class NTXentLossSegmV2(MemoryBankModule):
    """Implementation of the Contrastive Cross Entropy Loss.
    ...
    def __init__(
        ...
        super(NTXentLossSegmV2, self).__init__(size=memory_bank_size)
        self.temperature = temperature
        self.gather_distributed = gather_distributed
        self.cross_entropy = nn.CrossEntropyLoss(reduction='mean')
        self.eps = 1e-8
    ...
    def forward(self, out0: torch.Tensor, out1: torch.Tensor):
        ...
        # out1: shape: (batch_size, embedding_size)
        # negatives: shape: (embedding_size, memory_bank_size)
        out1, negatives = super(NTXentLossSegmV2, self).forward(
            out1, update=out0.requires_grad
        )
        ...
        if negatives is not None:
            ...
            if self.gather_distributed and dist.world_size() > 1:
                # gather hidden representations from other processes
                out0_large = torch.cat(dist.gather(out0.transpose(1, 2).transpose(2, 3).reshape(batch_size*h*w, -1)), 0)
                out1_large = torch.cat(dist.gather(out1.transpose(1, 2).transpose(2, 3).reshape(batch_size*h*w, -1)), 0)
                diag_mask = dist.eye_rank(batch_size*h*w, device=out0.device)
            else:
                # single process

                out0 = out0.transpose(1, 2).transpose(2, 3).reshape(batch_size*h*w, -1)
                out1 = out1.transpose(1, 2).transpose(2, 3).reshape(batch_size*h*w, -1)

                out0_large = out0
                out1_large = out1
                diag_mask = torch.eye(batch_size*h*w, device=out0.device, dtype=torch.bool)
            ...
            # remove simliarities between same views of the same image
            logits_00 = logits_00[~diag_mask].view(batch_size*h*w, -1)
            logits_11 = logits_11[~diag_mask].view(batch_size*h*w, -1)
            ...
            # create labels
            labels = torch.arange(batch_size* h * w, device=device, dtype=torch.long)
